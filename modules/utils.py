...
def format_currency(value: float, currency_format: str = 'CLP') -> str:
    """Format value as currency based on format string"""
    if currency_format == 'CLP':
        return f"$ {value:,.0f}".replace(",", "X").replace(".", ",").replace("X", ".")
    elif currency_format == 'USD':
        return f"${value:,.2f}"
    elif currency_format == 'EUR':
        return f"€{value:,.2f}"
    else:
        return f"{currency_format} {value:,.2f}"
